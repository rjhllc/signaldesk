...
def llm_refine_results(payload):
    settings = llm_settings()
    if not settings['configured']:
        raise LLMError('AI result filtering is not configured')
...
def x_search(payload, plan=None):
    token = os.environ.get('X_BEARER_TOKEN')
    if not token:
        raise RuntimeError('X_BEARER_TOKEN is not configured on the server')
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/health':
            ...
            self.send_json(200, {
                ...
                'service': 'signaldesk',
                'build': BUILD_VERSION,
                'token_configured': bool(os.environ.get('X_BEARER_TOKEN')),
                'llm_configured': settings['configured'],
                'llm_model': settings['model'] or None,
    ...
    def do_POST(self):
        if self.path not in {'/api/preview', '/api/report', '/api/llm-filter'}:
            self.send_error(404)
            return
        plan = None
        try:
            payload = self.read_json()
            if self.path == '/api/llm-filter':
                result = llm_refine_results(payload)
                self.send_json(200, {
                    'ok': True,
                    'build': BUILD_VERSION,
                    **result,
